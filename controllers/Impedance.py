...
class Impedance:
    def __init__(self, model, data, viewer) -> None:
        ...
        self.data = data
        self.viewer = viewer
        self.F_extL = np.zeros(6)  # Initialize external force attribute for left arm
        self.F_extR = np.zeros(6)  # Initialize external force attribute for right arm
    ...
    def armCrtl(self, JacP):
        ...
        # Calculate external forces (F_extL and F_extR)
        self.F_extL = self.data.cfrc_ext[self.site_idL][:3]
        self.F_extR = self.data.cfrc_ext[self.site_idR][:3]
