...
def main():
    ...
    # pre_grasp_pose_L = ([-0.15, 0.33, 0.52],[0, 0, 1, 0]) 
    pre_grasp_pose_L = ([-0.4, 0.33, 0.25],[1, 0, 1, 0])
    # pre_grasp_pose_R = ([0.03, 0.33, 0.52], [0, 1, 0, 0])
    pre_grasp_pose_R = ([0.3, 0.33, 0.17], [0, 1, 0, -1])



    final_pose_L = ([-0.2, 0.33, 0.25],[1, 0, 1, 0])
    # final_pose_L = ([-0.15, 0.33, 0.25],[0, 0, 1, 0])
    final_pose_R = ([0.05, 0.33, 0.17],[0, 1, 0, -1])
    # final_pose_R = ([0.03, 0.33, 0.25], [0, 1, 0, 0])
    ...
    while viewer.is_running():
        ...
        if(erL < tolerance and erR < tolerance and j <= 1500 and stage == 2):
            ...
            if(j == 1500):
                ...
                final_object_pose = ([0.0, 0.3,0.46], [0,0, 0, 1])
                # final_object_pose = ([0.3, 0.3,0.4], [0,0, 0, 1])
                # final_object_pose = ([0.2, 0.1,0.35], [0,0, 0, 1])
                # final_object_pose = ([0.0, 0.0,0.4], [1,0, 0, 1])
                # final_object_pose = ([0.0, 0.3,0.35],[0, 1, 0, 1])
