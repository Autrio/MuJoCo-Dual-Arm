...
from controllers.utils.RRT import generate_smooth_rrt
...
def main():
    ...
    DtrajL = generate_smooth_rrt(data.mocap_pos[controller.mocap_idL], [-0.0444176, -0.249729, 0.32524], 1500, obstacle_list)
    DtrajR = generate_smooth_rrt(data.mocap_pos[controller.mocap_idR], [-0.0395817, 0.259253 ,0.306346], 1500, obstacle_list)
    ...
    while viewer.is_running():
        if(erL < tolerance and erR < tolerance and i <= 1500 and stage == 1):
            ...
            if(i == 1500):
                ...
                AtrajL = generate_smooth_rrt(data.mocap_pos[controller.mocap_idL], [-0.15, 0.35, 0.5], 1500, obstacle_list1)
                AtrajR = generate_smooth_rrt(data.mocap_pos[controller.mocap_idR], [0.05, 0.35, 0.5], 1500, obstacle_list1)
